...
var server = http.createServer(function(request, response){
    ...
    if(path === '/'){
        ...
        let page = fs.readFileSync(`./db1.json`).toString()
        let array = JSON.parse(page)
        let pages = array.map(items=>`<li>${items.id}</li>`).join(``)
        string = string.replace(`{{page}}`,`<ul id="ul">${pages}</ul>`)
        ...
        response.write(fs.readFileSync(`./1.css`))
        response.end()
    }else if(path === '/3.html'){
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/html;charset=utf-8')
        ...
        response.statusCode = 200
        response.setHeader(`content-Type`, `text/json;charset=utf-8`)
        response.write(fs.readFileSync(`./db2.json`))
        response.end()
    } else if(path === `/db3.json`){
        response.statusCode = 200
        response.setHeader(`content-Type`, `text/json;charset=utf-8`)
        response.write(fs.readFileSync(`./db3.json`))
        response.end()
    } else {
